    ...
    const filteredVideos = videoSources.filter(video => generatedDays.includes(video.day)).sort((videoA, videoB) => {
        const dayA = parseInt(videoA.day.replace("day", ""));
        const dayB = parseInt(videoB.day.replace("day", ""));
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function playNextVideo() {
        ...
        if (currentVideoIndex < filteredVideos.length - 1) {
            let currentVideoDay = filteredVideos[currentVideoIndex].day.replace("day", "")
            currentVideoIndex++;
            loadAndPlayVideo(filteredVideos[currentVideoIndex].img);
            
            //Change Chapter Active Div::
            let nextVideoDay = filteredVideos[currentVideoIndex].day.replace("day", "")
